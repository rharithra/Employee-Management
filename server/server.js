...
const io = new Server(httpServer, {
  cors: {
    origin: process.env.CORS_ORIGIN || ['https://Employee-management-backend.onrender.com'],
    methods: ['GET', 'POST'],
    credentials: true
  }
});
...
const defaultUsers = [
  {
    ...
    kpi: {
      ...
      goalScore: 92,
      goalWeight: 25,
      baseIncentiveRate: 1000,
      maxIncentive: 50000,
      totalKPI: 88.75,
      incentiveAmount: 10000,
      calculatedDate: '2025-01-13T10:00:00.000Z',
...
// Debug goals endpoint
app.get('/api/debug/goals', (req, res) => {
  res.json({
    totalGoals: goals.length,
    allGoals: goals,
    goalEmployeeIds: goals.map(g => ({ id: g.id, employeeId: g.employeeId, type: typeof g.employeeId })),
...
app.get('/api/performance-goals', auth, (req, res) => {
  try {
    console.log('🎯 Goals API called by user:', req.user.id, 'role:', req.user.role);
    console.log('📝 Total goals in system:', goals.length);
    console.log('📊 All goals:', goals.map(g => ({ id: g.id, employeeId: g.employeeId, title: g.title })));
    
    let filteredGoals = goals;
    
    if (req.user.role === 'employee') {
      console.log('👤 Filtering goals for employee ID:', req.user.id);
      filteredGoals = goals.filter(g => g.employeeId === req.user.id);
      console.log('✅ Filtered goals for employee:', filteredGoals.length);
    } else if (req.user.role === 'manager') {
      const teamMemberIds = users.filter(u => u.manager === req.user.id).map(u => u.id);
      console.log('👥 Manager team member IDs:', teamMemberIds);
      filteredGoals = goals.filter(g => teamMemberIds.includes(g.employeeId));
      console.log('✅ Filtered goals for manager:', filteredGoals.length);
    }
...
app.post('/api/performance-goals', auth, (req, res) => {
  try {
    ...
    // Send notification to employee about new goal
    const employee = findUserById(employeeId);
    if (employee) {
      const notification = createNotification(
        employeeId,
        'goal_assigned',
        `New goal assigned: "${title}" - Target date: ${targetDate}`,
        { goalId: goal.id, type: 'goal_assignment' }
      );
      sendRealTimeNotification(employeeId, notification);
    }
...
app.put('/api/performance-goals/:id', auth, (req, res) => {
  try {
    ...
    // Check if goal is being completed by employee
    ...
    // Send notification to admin/manager when employee completes goal
    if (wasCompleted && isEmployee) {
      const employee = findUserById(goal.employeeId);
      const manager = findUserById(goal.setBy);
      
      if (manager) {
        const notification = createNotification(
          goal.setBy,
          'goal_completed',
          `${employee.firstName} ${employee.lastName} completed goal: "${goal.title}"`,
          { goalId: goal.id, employeeId: goal.employeeId, type: 'goal_completion' }
        );
        sendRealTimeNotification(goal.setBy, notification);
      }
      
      // Also notify all admins
      const admins = users.filter(u => u.role === 'admin' && u.id !== goal.setBy);
